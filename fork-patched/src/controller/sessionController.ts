...
// AchadinhosBot patch: mesmo raciocínio do miscController — resolve o caminho real do
// token e envolve chamadas potencialmente travadas do Puppeteer em timeout.
function resolveTokenPathSess(session: string, cfg: any): string {
  const folder = cfg?.folderNameToken || './tokens';
  return path.resolve(process.cwd(), folder, `${session}.data.json`);
}
async function withTimeoutSess<T>(p: Promise<T>, ms: number): Promise<T | null> {
...
export async function closeSession(req: Request, res: Response): Promise<any> {
  ...
  try {
    ...
    // AchadinhosBot patch: close() com timeout de 8s. Sem timeout, um Chromium wedged
    // trava o endpoint indefinidamente e o AchadinhosBot nunca consegue seguir para
    // clear-session-data → usuário fica preso em "não gera QR".
    if (req?.client?.close) {
      await withTimeoutSess(req.client.close(), 8000);
    }
    ...
    // Mesmo em erro, marca o slot como fechado para permitir recuperação no start-session seguinte.
    try { (clientsArray as any)[session] = { status: null }; } catch {}
    return res
      .status(500)
      .json({ status: false, message: 'Error closing session', error: error?.message || String(error) });
  }
}
...
export async function logOutSession(req: Request, res: Response): Promise<any> {
  ...
  const session = req.session;
  try {
    // AchadinhosBot patch: logout com timeout, e apagamos o token/UserDataDir SÍNCRONO
    // dentro do try, usando os caminhos corretos (o original usava
    // `__dirname + '../../../tokens/...'` — string quebrada — então nunca apagava nada).
    if (req?.client?.logout) {
      await withTimeoutSess(req.client.logout(), 8000);
    }
    deleteSessionOnArray(session);

    const pathUserData = (config.customUserDataDir || '/tmp/userDataDir/') + session;
    const pathTokens = resolveTokenPathSess(session, config);
    ...
    if (fs.existsSync(pathUserData)) {
      await fs.promises.rm(pathUserData, {
        ...
        force: true,
        retryDelay: 500,
      }).catch((e) => req.logger.warn(`[logOutSession] rm userDataDir failed: ${e?.message}`));
      removed.userDataDir = true;
    }
    if (fs.existsSync(pathTokens)) {
      await fs.promises.rm(pathTokens, {
        force: true,
      }).catch((e) => req.logger.warn(`[logOutSession] rm token failed: ${e?.message}`));
      removed.token = true;
    }

    req.logger.info(
      `[logOutSession] session=${session} userDataDir=${removed.userDataDir} token=${removed.token} tokenPath=${pathTokens}`
    );
    ...
    req.logger.error(error);
    // Fallback: mesmo com erro, tenta apagar token/userdata para não deixar sessão zumbi.
    try {
      deleteSessionOnArray(session);
      const pathUserData = (config.customUserDataDir || '/tmp/userDataDir/') + session;
      const pathTokens = resolveTokenPathSess(session, config);
      if (fs.existsSync(pathUserData)) {
        await fs.promises.rm(pathUserData, { recursive: true, force: true }).catch(() => {});
      }
      if (fs.existsSync(pathTokens)) {
        await fs.promises.rm(pathTokens, { force: true }).catch(() => {});
      }
    } catch {}
    return res
      .status(500)
