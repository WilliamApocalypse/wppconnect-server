...
// Se ficou preso em INITIALIZING por mais de 60 segundos,
// considera a sessão corrompida e libera nova tentativa.
if (
  client.status === 'INITIALIZING' &&
  client.initializingStartedAt &&
  Date.now() - client.initializingStartedAt > 60000
) {
  req.logger.warn(
    `[${session}] INITIALIZING preso por mais de 60 segundos. Limpando sessão.`
  );
...
client.status = "CLOSED";
  ...
  // AchadinhosBot patch: apaga token corrompido do disco. Sem isso o proximo create()
  // le o mesmo token quebrado e wedge em loop. Causa raiz dos usuarios sem QR.
  try {
    const _path = require('path');
    const _fs = require('fs');
    const folder = (req.serverOptions as any)?.folderNameToken || './tokens';
    const tokenPath = _path.resolve(process.cwd(), folder, `${session}.data.json`);
    if (_fs.existsSync(tokenPath)) {
      _fs.rmSync(tokenPath, { force: true });
      req.logger.warn(`[${session}] token corrompido removido: ${tokenPath}`);
    }
    const cudd = (req.serverOptions as any)?.customUserDataDir;
    if (cudd) {
      const userDataPath = cudd + session;
      if (_fs.existsSync(userDataPath)) {
        _fs.rmSync(userDataPath, { recursive: true, force: true, maxRetries: 3 });
        req.logger.warn(`[${session}] userDataDir removido: ${userDataPath}`);
      }
    }
  } catch (cleanupErr) {
    req.logger.warn(`[${session}] falha ao limpar token/userDataDir: ${cleanupErr}`);
  }
