...
export async function sendMessageHqLinkPreview(req: Request, res: Response) {
  const { phone, message, linkPreview } = req.body;

  if (!phone || !message || !linkPreview?.imageUrl) {
    return res.status(400).json({
      status: 'Error',
      message: 'phone, message and linkPreview.imageUrl are required',
    });
  }
    ...
    for (const contato of contacts) {
      results.push(
        await (req.client as any).page.evaluate(
          async ({ chatId, text, preview }) => {
            const WPP = (window as any).WPP;
            const WAPI = (window as any).WAPI;
            if (!WPP?.util?.generateThumbnailLinkPreviewData || !WPP?.chat?.sendTextMessage) {
              throw new Error('WA-JS link preview APIs are not available');
            }

            const hq = await WPP.util.generateThumbnailLinkPreviewData(preview.imageUrl);
            if (!hq?.thumbnailHQ || !hq?.mediaKey || !hq?.thumbnailDirectPath) {
              throw new Error('HQ link preview upload failed');
            }

            const sendResult = await WPP.chat.sendTextMessage(chatId, text, {
              waitForAck: true,
              linkPreview: {
                title: preview.title,
                description: preview.description || '',
                canonicalUrl: preview.canonicalUrl,
                matchedText: preview.matchedText || preview.canonicalUrl,
                richPreviewType: preview.richPreviewType ?? 0,
                doNotPlayInline: preview.doNotPlayInline ?? true,
                ...hq,
              },
            });

            const messageId = sendResult?.id;
            if (messageId && WAPI?.getMessageById) {
              const msg = await WAPI.getMessageById(messageId);
              return JSON.parse(JSON.stringify(msg || sendResult));
            }
            if (messageId && WPP.chat.getMessageById) {
              const msg = await WPP.chat.getMessageById(messageId);
              return JSON.parse(JSON.stringify(msg || sendResult));
            }
            return sendResult;
          },
          { chatId: `${contato}`, text: message, preview: linkPreview }
        )
      );
    }
